...
#include <../../include/fertilized/impurities/shannonentropy.h>
...
#pragma warning( disable : 4800 )
  ...
  auto *libfunc_result = new std::shared_ptr<ShannonEntropy<uint>>(
    new ShannonEntropy<
              uint
>(
          ));
#pragma warning( pop )
  ...
  // Unpack instance.
  auto __instance_unpacked = *static_cast<std::shared_ptr<ShannonEntropy<uint>>*>(__instance);
  // From C types conversions.
...
void delete_ShannonEntropy_uint(void *ptr) {
  auto * storage = static_cast<std::shared_ptr<ShannonEntropy<uint>>*>(ptr);
  delete storage;
}
