...
#pragma warning( disable : 4800 )
  ...
  auto *libfunc_result = new std::shared_ptr<Samme_R<uint8_t,uint8_t,uint,std::vector<float>,std::vector<float>>>(
    new Samme_R<
              uint8_t,
              uint8_t,
              uint,
...
#pragma warning( pop )
  ...
  // Unpack instance.
  auto __instance_unpacked = *static_cast<std::shared_ptr<Samme_R<uint8_t,uint8_t,uint,std::vector<float>,std::vector<float>>>*>(__instance);
  // From C types conversions.
...
void delete_Samme_R_uint8_uint8_uint_fv_fv(void *ptr) {
  auto * storage = static_cast<std::shared_ptr<Samme_R<uint8_t,uint8_t,uint,std::vector<float>,std::vector<float>>>*>(ptr);
  delete storage;
}
