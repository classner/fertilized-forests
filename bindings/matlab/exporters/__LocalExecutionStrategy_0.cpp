...
#pragma warning( disable : 4800 )
  ...
  auto *libfunc_result = new std::shared_ptr<LocalExecutionStrategy<int,int,uint,std::vector<float>,std::vector<float>>>(
...
void delete_LocalExecutionStrategy_i_i_uint_fv_fv(void *ptr) {
  auto * storage = static_cast<std::shared_ptr<LocalExecutionStrategy<int,int,uint,std::vector<float>,std::vector<float>>>*>(ptr);
  delete storage;
}
