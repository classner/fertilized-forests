...
#pragma warning( disable : 4800 )
  ...
  auto *libfunc_result = new std::shared_ptr<NoBagging<uint8_t,int16_t,uint,std::vector<float>,std::vector<float>>>(
    new NoBagging<
              uint8_t,
              int16_t,
              uint,
...
#pragma warning( pop )
  ...
  // Unpack instance.
  auto __instance_unpacked = *static_cast<std::shared_ptr<NoBagging<uint8_t,int16_t,uint,std::vector<float>,std::vector<float>>>*>(__instance);
  // From C types conversions.
...
void delete_NoBagging_uint8_int16_uint_fv_fv(void *ptr) {
  auto * storage = static_cast<std::shared_ptr<NoBagging<uint8_t,int16_t,uint,std::vector<float>,std::vector<float>>>*>(ptr);
  delete storage;
}
