...
#pragma warning( disable : 4800 )
  ...
  auto *libfunc_result = new std::shared_ptr<TwoSideClassificationThresholdOptimizer<float,float,uint>>(
...
#pragma warning( pop )
  ...
  // Unpack instance.
  auto __instance_unpacked = *static_cast<std::shared_ptr<TwoSideClassificationThresholdOptimizer<float,float,uint>>*>(__instance);
  // From C types conversions.
...
void delete_TwoSideClassificationThresholdOptimizer_f_f_uint(void *ptr) {
  auto * storage = static_cast<std::shared_ptr<TwoSideClassificationThresholdOptimizer<float,float,uint>>*>(ptr);
  delete storage;
}
