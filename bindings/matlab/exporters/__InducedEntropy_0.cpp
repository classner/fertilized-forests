...
#pragma warning( disable : 4800 )
  ...
  auto *libfunc_result = new std::shared_ptr<InducedEntropy<float>>(
    new InducedEntropy<
              float
>(
          p
...
#pragma warning( pop )
  ...
  // Unpack instance.
  auto __instance_unpacked = *static_cast<std::shared_ptr<InducedEntropy<float>>*>(__instance);
  // From C types conversions.
...
void delete_InducedEntropy_f(void *ptr) {
  auto * storage = static_cast<std::shared_ptr<InducedEntropy<float>>*>(ptr);
  delete storage;
}
