...
#pragma warning( disable : 4800 )
  ...
  auto *libfunc_result = new std::shared_ptr<Samme_R<float,float,uint,std::vector<float>,std::vector<float>>>(
    new Samme_R<
              float,
              float,
              uint,
...
#pragma warning( pop )
  ...
  // Unpack instance.
  auto __instance_unpacked = *static_cast<std::shared_ptr<Samme_R<float,float,uint,std::vector<float>,std::vector<float>>>*>(__instance);
  // From C types conversions.
...
void delete_Samme_R_f_f_uint_fv_fv(void *ptr) {
  auto * storage = static_cast<std::shared_ptr<Samme_R<float,float,uint,std::vector<float>,std::vector<float>>>*>(ptr);
  delete storage;
}
