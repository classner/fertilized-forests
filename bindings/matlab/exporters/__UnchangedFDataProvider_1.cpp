...
using namespace fertilized;
  ...
  // From C types conversions.
    ...
    fertilized::Array<int, 2, 2> __converted_data_array;
    {
      ...
      int dimidx;
      int *_dptr = __converted_data_array.getData();
      int *_iptr = reinterpret_cast<int*>(mxGetData(data_array));
...
#pragma warning( disable : 4800 )
  ...
  auto *libfunc_result = new std::shared_ptr<UnchangedFDataProvider<int,uint>>(
...
void delete_UnchangedFDataProvider_i_uint(void *ptr) {
  auto * storage = static_cast<std::shared_ptr<UnchangedFDataProvider<int,uint>>*>(ptr);
  delete storage;
}
