...
#pragma warning( disable : 4800 )
  ...
  auto *libfunc_result = new std::shared_ptr<TwoSideClassificationThresholdOptimizer<uint8_t,int,uint>>(
...
#pragma warning( pop )
  ...
  // Unpack instance.
  auto __instance_unpacked = *static_cast<std::shared_ptr<TwoSideClassificationThresholdOptimizer<uint8_t,int,uint>>*>(__instance);
  // From C types conversions.
...
void delete_TwoSideClassificationThresholdOptimizer_uint8_i_uint(void *ptr) {
  auto * storage = static_cast<std::shared_ptr<TwoSideClassificationThresholdOptimizer<uint8_t,int,uint>>*>(ptr);
  delete storage;
}
