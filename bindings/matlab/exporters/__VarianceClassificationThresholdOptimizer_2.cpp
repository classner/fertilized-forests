...
#pragma warning( disable : 4800 )
  ...
  auto *libfunc_result = new std::shared_ptr<VarianceClassificationThresholdOptimizer<float,int,uint>>(
...
#pragma warning( pop )
  ...
  // Unpack instance.
  auto __instance_unpacked = *static_cast<std::shared_ptr<VarianceClassificationThresholdOptimizer<float,int,uint>>*>(__instance);
  // From C types conversions.
...
void delete_VarianceClassificationThresholdOptimizer_f_i_uint(void *ptr) {
  auto * storage = static_cast<std::shared_ptr<VarianceClassificationThresholdOptimizer<float,int,uint>>*>(ptr);
  delete storage;
}
