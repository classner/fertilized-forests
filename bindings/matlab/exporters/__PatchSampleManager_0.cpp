...
using namespace fertilized;
  ...
  // From C types conversions.
   ...
   std::vector<fertilized::Array<const uint8_t, 3, 3>> __converted_images;
   {
    ...
    fertilized::Array<int16_t, 2, 2> __converted_patch_descs;
    {
      ...
      int dimidx;
      int16_t *_dptr = __converted_patch_descs.getData();
      int16_t *_iptr = reinterpret_cast<int16_t*>(mxGetData(patch_descs));
...
#pragma warning( disable : 4800 )
  ...
  auto *libfunc_result = new std::shared_ptr<PatchSampleManager<uint8_t,int16_t>>(
    new PatchSampleManager<
              uint8_t,
              int16_t
>(
          __converted_images,
...
void delete_PatchSampleManager_uint8_int16(void *ptr) {
  auto * storage = static_cast<std::shared_ptr<PatchSampleManager<uint8_t,int16_t>>*>(ptr);
  delete storage;
}
