...
BOOST_AUTO_TEST_CASE(Correctness_Boosting_Serialization) {
        ...
        uint n_trees = 200;
        decltype(soil.tree_vec_t()) adaBoostTrees;
        ...
        auto sammeForest = soil.ForestFromTrees(
                              sammeTrees,
                              soil.BoostedTraining(soil.Samme()));
        auto sammeDeserialized = serialize_deserialize(sammeForest);
        BOOST_REQUIRE(*sammeForest == *sammeDeserialized);

        decltype(soil.tree_vec_t()) sammeRTrees;
        for (uint i = 0; i < n_trees; ++i) {
        ...
        auto sammeRForest = soil.ForestFromTrees(
                                sammeRTrees,
                                soil.BoostedTraining(soil.Samme_R()));
...
double f1_score(Array<uint, 2, 2>& truth, Array<double, 2, 2>& prediction) {
    ...
    for(size_t i = 0; i < truth.size(); ++i) {
        values[truth[i][0]*2U+
          std::min(static_cast<uint>(prediction[i][1]-
                                     prediction[i][0] + 1.0), 1U)]++;
    }
...
std::pair<Array<float,2,2>, Array<uint,2,2> > make_spiral_fixed(
    uint n_samples_per_arm=1000,
    uint n_arms=2,
    float noise=0.1) {
#define M_PI 3.14159265358979323846
    ...
    for(uint arm_id = 0U; arm_id < n_arms; ++arm_id) {
        ...
        for(uint point_id = 0U; point_id < n_samples_per_arm; ++point_id) {
            ...
            Array<float,1,1> position = point_on_circle(angle +
              angle_add * static_cast<float>(M_PI), 1.f + 2.f*angle_add);
            position[0] += -0.152176 * noise;
            position[1] += 0.141444 * noise;
...
BOOST_AUTO_TEST_CASE(Correctness_Boosting_Result_AdaBoost) {
    ...
    for (uint i = 0U; i < n_trees; ++i) {
        ...
        auto rcto = soil.RandomizedClassificationThresholdOptimizer(1, 2,
            soil.EntropyGain(soil.ShannonEntropy()), 0, 1, 1+i);
        auto tClassifier = soil.ThresholdDecider(stdFeatureSelect,
            soil.AlignedSurfaceCalculator(), rcto);
    ...
    auto forest = soil.Forest(1, 1, 2, n_trees,
       cls, lm, soil.BoostedTraining(soil.AdaBoost()));
...
#ifdef _WIN32
    BOOST_CHECK(static_cast<uint>(f1_score(new_Y, predictions)*100000.f) >=
                98000);
#else
    BOOST_CHECK(static_cast<uint>(f1_score(new_Y, predictions)*100000.f) >=
                98000);
...
BOOST_AUTO_TEST_CASE(Correctness_Boosting_Result_Samme) {
    ...
    for (uint i = 0; i < n_trees; ++i) {
            ...
            auto rcto = soil.RandomizedClassificationThresholdOptimizer(
              1, 2, soil.EntropyGain(soil.ShannonEntropy()), 0, 1, 1+i);
            auto tClassifier = soil.ThresholdDecider(stdFeatureSelect,
               soil.AlignedSurfaceCalculator(), rcto);
            auto leafMgr = soil.ClassificationLeafManager(2);
    ...
    auto forest = soil.Forest(1, 1, 2, n_trees, cls, lm,
      soil.BoostedTraining(soil.Samme()));
...
#ifdef _WIN32
    BOOST_CHECK_EQUAL(static_cast<uint>(f1_score(new_Y, predictions)*100000.f),
                      97979);
#else
    BOOST_CHECK_EQUAL(static_cast<uint>(f1_score(new_Y, predictions)*100000.f),
                      97979);
